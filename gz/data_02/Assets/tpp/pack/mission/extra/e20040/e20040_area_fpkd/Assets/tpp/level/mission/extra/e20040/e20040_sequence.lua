...
this.RadioList = {
	...
	Radio_HeliHP70 = "e0040_rtrg0110", 
	Radio_HeliHP50 = "e0040_rtrg0111", 
	Radio_HeliHP10	= "e0040_rtrg0120", 
	...
	Radio_HeliDeadwithoutPlayerKill = "f0033_gmov0120",
...
this.DemoList = {
	Demo_Opening 		= "p12_030000_000",		
	Demo_Boss			= "p12_030020_000",
	Demo_MissionClear01	= "p12_030010_000",		
	Demo_MissionClear02	= "p12_030010_001",		
	Demo_MissionClear03	= "p12_030010_002",		
}
...
this.Messages = {
	...
	Character = {
		...
		{ data = "Tactical_Vehicle_WEST_001", message = "LeftWheelBroken", commonFunc = function() this.FriendVehicleWheelBroken() end },
		{ data = "Tactical_Vehicle_WEST_001", message = "RightWheelBroken", commonFunc = function() this.FriendVehicleWheelBroken() end },
		...
		{ data = "Player", message = "OnPickUpWeapon" , commonFunc = function() this.Common_PickUpWeaopn() end },	
	},
	Gimmick = {
...
this.OnShowRewardPopupWindow = function()
	...
	if( TppSequence.IsGreaterThan( sequence, "Seq_MissionClearDemo01" ) ) then
		...
		if ( ( Rank == 1 ) and ( Rank < this.tmpBestRank ) ) then
			...
			hudCommonData:ShowBonusPopupItem( "reward_clear_s_sniper", "WP_sr01_v00" ) 
		end

		
		if uiCommonData:IsHaveCassetteTape( "tp_chico_01" ) == false then	
		...
		if	TppGameSequence.GetGameFlag("hardmode") == false and					
			PlayRecord.GetMissionScore( 20040, "NORMAL", "CLEAR_COUNT" ) == 1 then	
...
this.MoaiCount = function()
	...
	if moai == true then
		...
		if count == 5 then
			PlayRecord.RegistPlayRecord( "MOAI_SHOT" )		
		end
...
local commonUiMissionSubGoalNo = function( id )
	...
	if( this.CounterList.SubGoalNumber ~= id ) then
		
		this.CounterList.SubGoalNumber = id
...
this.FriendVehicleWheelBroken = function()
	local vehicleObject = Ch.FindCharacterObjectByCharacterId( "Tactical_Vehicle_WEST_001" )
	local vehicle = vehicleObject:GetCharacter()
	local plgLife = vehicle:FindPluginByName( "Life" )
...
local commonSetup = function ()
	...
	WeatherManager.SetCurrentClock(5,31,0)
	TppClock.Stop()
...
this.OnSkipLeaveCommon = function(manager)
	...
	elseif( sequence == "Seq_Snipe" )  then
		...
		TppEnemyUtility.ChangeStatus( "FriendMan" , "Faint" )
		TppEnemyUtility.SetLifeFlagByCharacterId( "FriendMan", "NoRecoverFaint" )
		TppEnemyUtility.SetLifeFlagByCharacterId("FriendMan", "NoDamageFaint" )
...
this.Seq_FreeTarget = {
	...
	OnEnter = function()
		...
		TppEnemyUtility.SetEnableCharacterId( "FriendMan", true )
		TppEnemyUtility.SetFlagSendMessageOnReceiveDamage( "FriendMan", true )
		TppEnemyUtility.SetLifeFlagByCharacterId( "FriendMan", "NoDamageSleep" ) 
		TppEnemyUtility.SetLifeFlagByCharacterId( "FriendMan", "NoDamageFaint" ) 
		TppData.Enable( "Tactical_Vehicle_WEST_001" )
...
this.Seq_AAGun = {
	...
	DeadWatcher01 = function()
		...
		if count == 5 then
			...
			if( TppMission.GetFlag( "isFriendManFaint" ) == true ) then 
	...
	FriendVehicleMovement = function()
		...
		if( RouteName ==  GsRoute.GetRouteId("VehicleRoute02")	) then
			...
			elseif(RoutePointNumber == 28 ) then
				...
				TppEnemyUtility.UnsetLifeFlagByCharacterId("FriendMan", "NoDamageFaint" )
				TppEnemyUtility.SetLifeFlagByCharacterId( "FriendMan", "NoRecoverFaint" )
				TppEnemyUtility.ChangeStatus( "FriendMan", "Faint" )
				TppEnemyUtility.SetLifeFlagByCharacterId("FriendMan", "NoDamageFaint" )
				...
				if( TppMission.GetFlag( "isDeadWatcher01END" ) == true ) then
					GkEventTimerManager.Start( "Timer_SnipeEnemyClear01", 1.5 )
				end
			end
		end
...
this.Seq_Snipe = {
	...
	OnEnter = function()
		...
		TppEnemyUtility.SetFlagSendMessageOnReceiveDamage( "FriendMan", true )
		GkEventTimerManager.Start( "Timer_SnipeEnemyGen01", 7 ) 
	...
	SnipeEnemyGroupeGen01 = function()
		...
		TppCommandPostObject.GsSetRealizeEnable( this.cpID, "FollowVehicleEnemy04", false ) 
	end,
	...
	RocketManVanish = function()
		...
		if( RouteName ==  GsRoute.GetRouteId("SnipeRoute05") ) then
			if (RoutePointNumber == 6) then
				...
				TppEnemyUtility.ChangeStatus( "SnipeEnemy05" , "Faint" ) 
...
this.Seq_Boss = {
	...
	OnEnter = function()
		...
		TppVehicleUtility.SetCannotRide("Tactical_Vehicle_WEST_001")
		TppVehicleUtility.SetCannotRide("Tactical_Vehicle_WEST_005")
		TppVehicleUtility.SetCannotRide("Truck_WEST03")
		...
		local RoutePointNumber				= TppData.GetArgument(1)
			...
			if( RouteName ==  GsRoute.GetRouteId("BossRoute02")  ) then
				if( RoutePointNumber == 0 ) then
					...
					plgLifeTruck:RequestSetLifeMax( "LeftWheelThirdLife" ) 
					plgLifeTruck:RequestSetLifeMax( "RightWheelThirdLife" ) 
					plgLifeTruck:RequestSetLifeMax( "LeftWheelFourthLife" ) 
					plgLifeTruck:RequestSetLifeMax( "RightWheelFourthLife" ) 
				elseif( RoutePointNumber == 1 ) then
	...
	HighSpeedSound = function()
		...
		TppSoundDaemon.PostEvent( 'sfx_s_demo_zoom' )		
	end,
	...
	KILLBOSS = function()
			...
			if( TppMission.GetFlag( "isAdditionalEnemyForceDeactive" ) == false ) then
				...
				if ( TppMission.GetFlag( "isInsideHeliDropArea" ) == false) then
					TppSupportHelicopterService.ChangeRoute("HeliBoarding_2")
				elseif ( TppMission.GetFlag( "isInsideHeliDropArea" ) == true) then
					TppSupportHelicopterService.ChangeRoute("HeliBoarding_2")
				end
			end
	end,
...
this.Seq_MissionFailed_HelicopterDead = {
	...
	OnEnter = function()
		...
		this.CounterList.GameOverRadioName = "Radio_PlayerDeadwithHeli"	
	end,
...
this.Seq_MissionFailed_HelicopterDeadNotOnPlayer = {
	...
	localChangeSequence = function()
		...
		if characterId == "Player" then
				this.CounterList.GameOverRadioName = "Radio_HeliDeadwithoutPlayerKill"	
		else
				this.CounterList.GameOverRadioName = "Radio_HeliDeadwithoutPlayer"	
		end
		TppSequence.ChangeSequence( "Seq_MissionGameOver" )
	end,
}
