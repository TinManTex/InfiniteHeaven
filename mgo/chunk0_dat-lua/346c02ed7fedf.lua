...
e:AddDynamicProperty("String","MarkerName")
  e.MarkerName="A"
  e:AddDynamicProperty("String","GenericPOITag")
  e.GenericPOITag="GENERIC_01"
  e:AddDynamicProperty("EntityLink","SoundObject")
  e.SoundObject=nil
end
function this.RemoveParm(e)
e:RemoveDynamicProperty"MarkerName"
e:RemoveDynamicProperty"GenericPOITag"
e:RemoveDynamicProperty"SoundObject"
end
function this.Construct(a)a:GetTable().capturePointFrontend=this
  ...
  if not i then
    ...
    local e={type=TppPoi.POI_TYPE_GENERIC,tags={"DOMINATION","CONTROL_POINT",e}}
    i=Utils.POIinitialize(a,e)
  end
  l=nil
  t=0
  n=0
...
function this.Destruct(l)
Utils.POIdestruct(l,i)
i=nil
...
function this.ProcessSignal(o,e)
  local c=false
  local n=nil
  local a=nil
  local d=false
...
function this.ValidateCapturers(e,o)
  local e=MpRulesetManager.GetActiveRuleset()
...
function s(l,o,c,d,f,n,e)
  if e then
    ...
    local t={}
    t.data=math.floor(.5+(n*255))
    local i=G
    local n=T
    local r=P
    local s=0
    if(Entity.IsNull(activeRuleset)or o==nil)or localPlayerTeam==nil then
      i=M
    elseif o==localPlayerTeam then
      i=y
    end
    if(Entity.IsNull(activeRuleset)or c==nil)or localPlayerTeam==nil then
      n=0
    elseif c==localPlayerTeam then
      n=S
    end
    if(Entity.IsNull(activeRuleset)or d==nil)or localPlayerTeam==nil then
      r=0
    elseif d==localPlayerTeam then
      r=E
    end
    if f then
      s=N
    ...
    l:EnableMarker(l:GetParm().MarkerName)
    l:UpdateMarker(t)
  else
    l:DisableMarker()
  end
...
function f(a,t,n)
  ...
  if not Entity.IsNull(e)then
    ...
    if not Entity.IsNull(e)then
      if e:IsPlaying()then
        if t==nil or t==localPlayerTeam then
