...
function this.OnAllocate(t)
  ...
  TppSave.WaitingAllEnqueuedSaveOnStartMission()TppGameStatus.Set("Mission","S_IS_ONLINE")
  Mission.Start()
  ...
  do
    ...
    p(n,o)TppScriptVars.DeclareSVars(n)TppScriptVars.SetSVarsNotificationEnabled(false)
    while f()do
      coroutine.yield()end
...
this.SetUpdateFunction(t)this.SetMessageFunction(t)TppClock.Start()end
function this.SetUpdateFunction(e)s={}l=0
  T={}r=0
  n={}c=0
...
function this.OnEnterMissionPrepare()if TppMission.IsMissionStart()then
  TppScriptBlock.PreloadSettingOnMissionStart()end
TppScriptBlock.ReloadScriptBlock()end
function this.OnTextureLoadingWaitStart()StageBlockCurrentPositionSetter.SetEnable(false)end
function this.OnMissionCanStart()if TppMission.IsMissionStart()then
...
function this.OnMissionGameStart(e)if not mvars.seq_demoSequneceList[mvars.seq_missionStartSequence]then
...
function this.ClearStageBlockMessage()StageBlock.ClearLargeBlockNameForMessage()StageBlock.ClearSmallBlockIndexForMessage()end
function this.ReservePlayerLoadingPosition(t,a,o,i,n,s)TppMain.DisableGameStatus()if t==TppDefine.MISSION_LOAD_TYPE.MISSION_FINALIZE then
  if i then
...
this.StageBlockCurrentPosition()end
function this.StageBlockCurrentPosition()if vars.initialPlayerFlag==PlayerFlag.USE_VARS_FOR_INITIAL_POS then
...
function this.OnReload(t)for n,e in pairs(t)do
  if i(e.OnLoad)then
    e.OnLoad()end
  if i(e.Messages)then
    t[n]._messageExecTable=Tpp.MakeMessageExecTable(e.Messages())end
...
this.SetUpdateFunction(t)this.SetMessageFunction(t)end
function this.OnUpdate(e)local e
  local i=s
  local t=T
  local e=n
  for e=1,l do
  ...
  for e=1,r do
    t[e]()end
  M()end
function this.OnChangeSVars(n,i,t)for n,e in ipairs(this._requireList)do
  if _G[e]then
...
function this.SetMessageFunction(t)d={}a=0
  u={}o=0
  for t,e in ipairs(this._requireList)do
    if _G[e]then
...
function this.OnMessage(e,t,i,s,n,p,l)local e=mvars
  local r=""local T
...
function this.OnTerminate(e)if e.sequence then
