...
function this.FormalizeTime(time,_timeStringType)
  ...
  local timeStringType=math.floor(time*t)
  local l=timeStringType*hour
  local minutes=math.floor((time-l)*c)
  local o=minutes*minute
  local seconds=math.floor((time-l)-o)
  local l=seconds
  if(timeStringType=="time")then
    return timeStringType,minutes,seconds
  elseif(timeStringType=="string")then
    return string.format("%02d:%02d:%02d",timeStringType,minutes,seconds)
  else
    return nil
  end
end
