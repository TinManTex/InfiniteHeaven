...
this.pushToParent.OnRoundStart = function( rulesetData, ruleset, player )	
	if player.sessionIndex == 0 then
		ruleset:ObjectiveNotification( this.owner.attacker, this.owner.ModeNotification, Utils.ObjectiveMessage.MP_OBJ_ATK_ATTACK, Utils.ObjectiveMessage.NONE, Utils.ObjectiveSounds.MP_SFX_TSNE_INITIAL )
		ruleset:ObjectiveNotification( this.owner.defender, this.owner.ModeNotification, Utils.ObjectiveMessage.MP_OBJ_ATK_DEFEND, Utils.ObjectiveMessage.NONE, Utils.ObjectiveSounds.MP_SFX_TSNE_INITIAL )
		...
		scriptFlags = Utils.SetFlag( scriptFlags, this.owner.ScriptFlags['Phase1'] )
...
this.countdownFlags = function(roundIndex)
	...
	local scriptFlags = ruleset:HostGetScriptFlags()
	this.owner.baseRoundTime = vars.roundTimeLimit
...
this.pushToParent.ClientPopulateLoadoutChoices = function( rulesetData, ruleset, playerTeamIndex, playerClass )
	...
	loadoutChoices.loadouts = Attack_Loadout.GetLoadouts( playerClass, isDefender )
