...
this.pushToParent.OnRoundStart = function( rulesetData, ruleset, player )	
	...
	if player.sessionIndex == 0 then
		...
		ruleset:ObjectiveNotification( this.owner.attacker, Utils.ObjectiveMessage.MP_OBJ, Utils.ObjectiveMessage.MP_OBJ_DOM_ATK, Utils.ObjectiveMessage.NONE, Utils.ObjectiveSounds.MP_SFX_DOM_INITIAL )
		ruleset:ObjectiveNotification( this.owner.defender, Utils.ObjectiveMessage.MP_OBJ, Utils.ObjectiveMessage.MP_OBJ_DOM_DEF, Utils.ObjectiveMessage.NONE, Utils.ObjectiveSounds.MP_SFX_DOM_INITIAL )
...
this.announceObjective = function()
	TppUiCommand.AnnounceLogViewLangId( "mgo_ui_announcer_DOM" )
end

this.SetInitialScore = function(rulesetData)
	local ruleset = MpRulesetManager.GetActiveRuleset()
	ruleset:IncrementStatByTeamIndex( this.owner.defender, MgoStat.STAT_COMTIX, ruleset.data.DominationTicketCount )
...
this.pushToParent.ClientPopulateLoadoutChoices = function( rulesetData, ruleset, playerTeamIndex, playerClass )
	...
	loadoutChoices.loadouts = Domination_Loadout.GetLoadouts( playerClass )
