...
e.MISSION_PREPARE_STATE=Tpp.Enum{"START","WAIT_INITALIZE","WAIT_TEXTURE_LOADING","END_TEXTURE_LOADING","WAIT_ALL_MEMBER_IF_COOP","WAIT_BASE_DEFENSE_RESULT","WAIT_SAVING_FILE","END_SAVING_FILE","WAIT_INITIAL_LOAD","END_CHECK_EXCEPTION","FINISH"}
local function s(n)
...
function e.SetNextSequence(r,n)
local s=nil
...
local e=e.GetCurrentSequenceName()Mission.SetMiniText(0,r)
end
...
if Tpp.IsQARelease()or nil then
local n=TppMission.GetMissionName()
local e=e.GetCurrentSequenceName()
end
end
...
n[e]={name=i,type=TppScriptVars.TYPE_UINT32,value=d(s),save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}
elseif t=="table"then
n[e]=s
end
e=e+1
...
e.SetNextSequence"Seq_Accepted_Invitation"elseif not a.Seq_Mission_Prepare.CanMissionGameStart()then
t("Timer_WaitStartingGame",s)
...
TppUI.FinishLoadingTips()t("Timer_WaitStartingGame",s)
else
...
local n=e.SYS_SEQUENCE_LENGTH+1
...
if Tpp.IsQARelease()or nil then
local e=e.GetSequenceNameWithIndex(mvars.seq_missionStartSequence)Mission.SetMiniText(0,e)
end
end
function e.SetOnEndMissionPrepareFunction(e)
mvars.seq_onEndMissionPrepareFunction=e
end
function e.DoOnEndMissionPrepareFunction()NamePlateMenu.SetDefaultNamePlateIfSetBegginerNamePlate()
if mvars.seq_onEndMissionPrepareFunction then
mvars.seq_onEndMissionPrepareFunction()
end
end
function e.IsFirstLandStart()
if(not mvars.seq_demoSequneceList[mvars.seq_missionStartSequence])and(mvars.seq_missionStartSequence==(e.SYS_SEQUENCE_LENGTH+1))then
return true
else
return false
end
...
if(not mvars.seq_demoSequneceList[mvars.seq_missionStartSequence])and(e.GetContinueCount()>0)then
return true
else
return false
end
...
function e.OnMessage(t,s,n,o,a,r,i)
