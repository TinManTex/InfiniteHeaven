...
function this._MakeMissionPackPathInfo(missionName,missionType)
  ...
  local AddCommonPackForMission=function(missionCode)
    this._AddCommonMissionPack(missionCode,missionTypeStr,missionName)
  end
  return AddCommonPackForMission
end
for locationName,locationEnum in pairs(locationEnums)do
  ...
  if addToList then
    this.MISSION_LIST_FOR_LOCATION[locationName]={}
  end
end
for i,missionDef in ipairs(this.MISSION_DEFINE_LIST)do
  ...
  if(not Tpp.IsMaster())or(Tpp.IsMaster()and missionType~=missionTypes.DEBUG)then
    ...
    local locationEnum=missionDef.location
    ...
    if isFlagMission or isDefMission then
      local locationName
      if locationEnum==locationEnums.SSD_AFGH then
        locationName="afgh"
      elseif locationEnum==locationEnums.MAFR then
        locationName="mafr"
      elseif locationEnum==locationEnums.SBRI then
        locationName="sbri"
      end
      if locationName then
        local missionDef={}
        missionDef.name=missionDef.name
        missionDef.pack=pack
        missionDef.location=locationName
        if isFlagMission then
          table.insert(this.FLAG_MISSION_LIST,missionDef)
        else
          table.insert(this.BASE_DEFENSE_LIST,missionDef)
        end
      end
    end
    local isEventMission=(missionType==missionTypes.EVENT)
    if not isEventMission then
      local locationNameU=locationNames[locationEnum+1]
      table.insert(this.MISSION_LIST_FOR_LOCATION[locationNameU],missionCodeStr)
    end
    ...
    if not isEventMission then
      ...
      if locationEnum==locationEnums.SSD_AFGH then
        ...
        locationName=string.lower(locationNames[locationEnum+1])
      end
...
function this.GetZombiePackagePath(missionCode)
  ...
  local packInfoForLocation=this.ZOMBIE_PACK_LIST[locationName]
  if not packInfoForLocation then
    return
  end
  local packPaths={}
  local currentStorySequence=TppStory.GetCurrentStorySequence()
  for i,packInfo in ipairs(packInfoForLocation)do
    if packInfo.loadCondition and packInfo.loadCondition(currentStorySequence,missionCode)then
      local pack=packInfo.pack
      if Tpp.IsTypeString(pack)then
        pack={pack}
      end
      if Tpp.IsTypeTable(pack)then
        for i,packPath in ipairs(pack)do
          table.insert(packPaths,packPath)
        end
      end
...
function this.AssignNpcInfosToGameObjectType(npcPackInfos)
  for i,npcInfo in ipairs(npcPackInfos)do
    SsdNpc.AssignInfosToGameObjectType{gameObjectType=npcInfo[1],npcType=npcInfo[2],partsType=npcInfo[3]}
  end
end
function this.GetNpcPackagePathList(npcPackInfos)
  local packPaths={}
  for i,npcInfo in ipairs(npcPackInfos)do
    local s=SsdNpc.GetGameObjectPackFilePathsFromPartsType{partsType=npcInfo[3]}
    for e,packPath in ipairs(s)do
      table.insert(packPaths,packPath)
    end
  end
  return packPaths
...
function this._AddExtraSmallPack(packNum)
  local packPaths=missionSmallPackTable[packNum]
...
function this.UpdateMissionListForDlcMission()
  ...
  for i,missionCodeStr in ipairs(dlcMissioncodeList)do
    table.insert(this.MISSION_LIST,tostring(missionCodeStr))
  end
  this.MISSION_ENUM=TppDefine.Enum(this.MISSION_LIST)
  if Mission.RegisterMissionCodeList then
    Mission.RegisterMissionCodeList{codeList=this.MISSION_LIST}
