...
this.MISSION_DEFINE_LIST={
  {type=missionTypes.FREE,name="f30010",location=locationEnums.SSD_AFGH,
    pack=function(s)
      this._AddCommonMissionPack(s,"free",
        "f30010")
...
function this._MakeMissionPackPathInfo(p,e)
  ...
  local a=function(s)
    this._AddCommonMissionPack(s,c,p)
  end
  return a
end
  ...
  if(not Tpp.IsMaster())or(Tpp.IsMaster()and _~=missionTypes.DEBUG)then
    ...
    if i or n then
      ...
      if o==locationEnums.SSD_AFGH then
        p="afgh"elseif o==locationEnums.MAFR then
        p="mafr"elseif o==locationEnums.SBRI then
        p="sbri"end
      if p then
...
function this.GetZombiePackagePath(p)
  local s=TppLocation.GetLocationName()
  if not s then
    return
  end
  local s=this.ZOMBIE_PACK_LIST[s]
  if not s then
    return
  end
  local c={}
  ...
  for s,e in ipairs(s)do
    if e.loadCondition and e.loadCondition(o,p)then
      local s=e.pack
...
function this.ResetAssinedDefaultInfosFromGameObjectType()SsdNpc.ResetAssignedInfosFromGameObjectType{gameObjectType=TppGameObject.GAME_OBJECT_TYPE_INSECT_1}SsdNpc.ResetAssignedInfosFromGameObjectType{gameObjectType=TppGameObject.GAME_OBJECT_TYPE_INSECT_2}SsdNpc.ResetAssignedInfosFromGameObjectType{gameObjectType=TppGameObject.GAME_OBJECT_TYPE_INSECT_3}SsdNpc.ResetAssignedInfosFromGameObjectType{gameObjectType=TppGameObject.GAME_OBJECT_TYPE_BOSS_1}SsdNpc.ResetAssignedInfosFromGameObjectType{gameObjectType=TppGameObject.GAME_OBJECT_TYPE_BOSS_2}SsdNpc.ResetAssignedInfosFromGameObjectType{gameObjectType=TppGameObject.GAME_OBJECT_TYPE_BOSS_3}
end
