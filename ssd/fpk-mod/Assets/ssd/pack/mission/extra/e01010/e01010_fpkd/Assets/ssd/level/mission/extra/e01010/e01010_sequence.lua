-- DOBUILD: 1
-- ORIGINALQAR: chunk0
-- PACKPATH: \Assets\ssd\pack\mission\extra\e01010\e01010.fpkd
local this = {}
...
sequences.Seq_Demo_OpenAvatarEdit = {
	...
	OnUpdate = function()
		...
		if not PlayerInfo.OrCheckStatus{ PlayerStatus.PARTS_ACTIVE } then
		  if not Tpp.IsMaster() then--tex added check
...
sequences.Seq_Game_SelectGender = {
	...
	OnEnter = function()
    local enableGenderSelect=Ivars.avatar_enableGenderSelect:Get()==1 --tex>
    if enableGenderSelect then
      Player.SetPadMask {
        settingName = "avatar_edit_mission",
        except = true,
        sticks = PlayerPad.STICK_R,
      ...
      TppUI.FadeIn( TppUI.FADE_SPEED.FADE_NORMALSPEED, "FadeInOnOpenAvatarEdit", TppUI.FADE_PRIORITY.SYSTEM )
		...
		if enableGenderSelect then--tex>
