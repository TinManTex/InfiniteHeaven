...
local function OnEndSaveLoading()
...
function this.OnLoad()
	...
	TppSequence.RegisterSequences{
		...
		"Seq_Demo_LoadSaveDataFromServer",
		"Seq_Demo_Init",
		"Seq_Demo_StartTitle",
...
function this.MissionPrepare()
	...
	if not this.IsWindowsEditor() and Tpp.IsQARelease() then
		
		gvars.DEBUG_initMissionToTitle = true
	end
...
sequences.Seq_Demo_WaitCopyRightLogo = {
	OnUpdate = function(self)
		...
		if not screen then
			...
			local konamiLogoScreenId = SplashScreen.Create("konamiLogo", "/Assets/ssd/ui/texture/logo/common_konami_logo_clp_nmp.ftex", 640, 640);
			
			
			
			local foxLogoScreenId = SplashScreen.Create("foxLogo", "/Assets/ssd/ui/texture/logo/common_fox_logo_clp_nmp.ftex", 640, 640);
...
local function CreateOrLoadSaveData()
	...
	local function ApplyConfig()
		...
		while GameConfig.IsBusyApplying() do
			DebugPrintState("Apply Config")
			coroutine.yield()
		end
	end
	...
	local loadCheckFuncs = {
		...
		[SAVE_FILE_PERSONAL] = function()
			TppSave.CheckPersonalSaveDataLoadResult()
		end,
		[SAVE_FILE_GAME] = function()
	...
	gvars.gameDataLoadingResult = TppDefine.SAVE_FILE_LOAD_RESULT.NOT_EXIST_FILE
	gvars.personalDataLoadingResult = TppDefine.SAVE_FILE_LOAD_RESULT.NOT_EXIST_FILE

	while TppScriptVars.IsSavingOrLoading() do
		DebugPrintState( "saving wait" )
		coroutine.yield()
	end
	...
	if not fileExists[SAVE_FILE_CONFIG] then
		...
		VoiceSettingMenu.OpenVoiceSettingMenu()
		while VoiceSettingMenu.IsOpenVoiceSettingMenu() do
			DebugPrintState("select voice")
			coroutine.yield()
		end
	...
	if not fileExists[SAVE_FILE_PERSONAL] then
		
		StoredParameterSystem.StoreInit( TppDefine.PARAMETER_FILE_PATH )
	end
	...
	if fileLoaded[SAVE_FILE_PERSONAL] then
		TppScriptVars.LoadVarsFromSlot( TppDefine.SAVE_SLOT.PERSONAL, TppScriptVars.GROUP_BIT_VARS, TppScriptVars.CATEGORY_PERSONAL )
	end

	local function GetNextSequence( result )
		if result == TppDefine.SAVE_FILE_LOAD_RESULT.OK then
			return "Seq_Demo_SelectGate"
	...
	if fileExists[SAVE_FILE_PERSONAL] and not fileCorrupt[SAVE_FILE_PERSONAL] then
		local result = gvars.personalDataLoadingResult
		local nextSequence = GetNextSequence( result )
		if nextSequence ~= "Seq_Demo_SelectGate" then
			return nextSequence
		end
	end
			...
			UI = {
				{
					...
					func = function()
						TppSequence.SetNextSequence("Seq_Demo_SelectGate")
					end,
				},
			},
		}
...
sequences.Seq_Demo_SelectGate = {
	OnEnter = function( self )
		...
		if true then
			...
			local itemList = {
				{ title = "External Connect",	OnSelect = function() DebugMenu.SetDebugMenuValue( "TppNetwork", "ConnectDebugGate", 1 ) end, },
				{ title = "KONAMI LAN",			OnSelect = function() DebugMenu.SetDebugMenuValue( "TppNetwork", "ConnectDebugGate", 2 ) end, },
			}

			local delegate = {}
			function delegate.GetItemCount(selector)
				return #itemList	
			end
			function delegate.GetItemTitle(selector, index)
				return itemList[index].title	
			end
			function delegate.DidSelectItem(selector, index)
				itemList[index].OnSelect(selector, index)	
				TppSequence.SetNextSequence("Seq_Demo_LogInKonamiServer")
			end
			function delegate.DidSelectBack(selector, index)
				
			end
			local selector = DebugSelector.Create("SELECT GATE", delegate)
			DebugSelector.Push(selector)
		end
	end,
	OnLeave = function( self )
...
sequences.Seq_Demo_LogInKonamiServer = {
			...
			Network = {
				{
					...
					func = function()
						if this.IsWindowsEditor() then
							TppSequence.SetNextSequence("Seq_Demo_LoadSaveDataFromServer")
						else
	...
	OnEnter = function( self )
		...
		if this.IsWindowsEditor() then
			...
			if Preference then
				local pref = Preference.GetPreferenceEntity( "TppNetworkDebugIdPreference" )
				if not Entity.IsNull( pref ) then
					if pref.account0_id ~= "" and pref.isAutoLogin == true then
...
sequences.Seq_Demo_WaitSavingPersonalData = {
	...
	OnUpdate = function( self )
		...
		if readResult == TppScriptVars.RESULT_OK then
...
this.serverSaveLoadCoroutine = nil

local function LoadServerSaveData()
	if this.IsWindowsEditor() then
	...
	while not SsdSaveSystem.IsIdle() do
		DebugPrintState("Connect Server for Loading Save Data...")
		coroutine.yield()
	end
...
sequences.Seq_Demo_LoadSaveDataFromServer = {
	OnEnter = function(self)
		self.nextSequence = nil
		this.serverSaveLoadCoroutine = coroutine.create(LoadServerSaveData)
	end,
	OnLeave = function(self)
		this.serverSaveLoadCoroutine = nil
	end,
	OnUpdate = function(self)
		if this.serverSaveLoadCoroutine then
			local status, ret1 = coroutine.resume(this.serverSaveLoadCoroutine)
			if not status then
				
				Fox.Log("init_sequence", string.format("error on LoadServerSaveData. %s", tostring(ret1)))
				self.nextSequence = "Seq_Demo_Init"
				this.serverSaveLoadCoroutine = nil
				return
			end
			if coroutine.status(this.serverSaveLoadCoroutine) == "dead" then
...
sequences.Seq_Demo_StartTitle = {
	OnEnter = function(self)
		...
		elseif ( Tpp.IsQARelease() or DEBUG ) and MissionTest and MissionTest.qaReleaseEntryFunction then 
...
sequences.Seq_Demo_ShowKonamiAndFoxLogo = {
	OnEnter = function(self)
		...
		local skipLogo = gvars.ini_isFirstLogin
		if DebugMenu then
			skipLogo = skipLogo or DebugMenu.GetDebugMenuValue( " <Ssd>", "SkipKonamiLogo" )
		end
		if skipLogo then
		...
		local function StateCallbackFox(screenId, state)
			if state == SplashScreen.STATE_DELETE then
				Fox.Log("foxLogoScreen is deleted. show fox logo.")
				this._StartPreTitleSequence()
			end
		end

		
		local function StateCallbackKonami(screenId, state)
			if state == SplashScreen.STATE_DELETE then
				Fox.Log("konamiLogoScreen is deleted. show fox logo.")
				SplashScreen.SetStateCallback( foxLogoScreenId, StateCallbackFox )
				SplashScreen.Show( foxLogoScreenId, 0.33, 2.0, 0.33 )
			end
		end
		SplashScreen.SetStateCallback( konamiLogoScreenId, StateCallbackKonami )
		SplashScreen.Show( konamiLogoScreenId, 0.33, 2.0, 0.33 )
	end,
