...
this.gameId="SSD"
this.gameDirectory="METAL GEAR SURVIVE"--DEBUGNOW
this.gameProcessName="mgv"--DEBUGNOW
...
function this.GetStringRef(strReference)
  ...
  if module==nil then
    InfCore.Log("WARNING GetStringRef: could not find module "..tostring(split[1]),false,true)
    return nil,nil
  end
...
if error then
  ...
  if this.paths.saves then--DEBUGNOW rethink
