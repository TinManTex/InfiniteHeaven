--InfSsdDebugStuff.lua --DEBUGNOW WIP
local this={}
...
function this.QARELEASE_DEBUG_AddProductForPacing()
  ...
  local function GetDebugStorySequence(e)
    return SsdStorySequenceList.DEBUG_storySequenceTable[e+1]
  end
  local currentStorySequence=this.GetCurrentStorySequence()
  for i=TppDefine.STORY_SEQUENCE.STORY_START,currentStorySequence do
    local e=GetDebugStorySequence(i)
    if e then
      if e.Equip then
      ...
      if e.overrideVarsFunction and(i==currentStorySequence)then
