...
function this.MakeMessageExecTable(n)
  ...
  local e={}
  local f=StrCode32"msg"
  local d=StrCode32"func"
  local u=StrCode32"sender"
  local _=StrCode32"option"
  for n,l in pairs(n)do
    e[n]=e[n]or{}
...
function this.PatchDlcCheckCoroutine(t,l,i,e)
  ...
  if PatchDlc.IsCheckingPatchDlc()then
    if not i then
      ...
      local e=n[e]TppUiCommand.SetPopupType"POPUP_TYPE_NO_BUTTON_NO_EFFECT"TppUiCommand.ShowErrorPopup(e)
    end
    while PatchDlc.IsCheckingPatchDlc()do
      n"waiting checking PatchDlc end..."coroutine.yield()TppUI.ShowAccessIconContinue()
    end
  end
  r()
