...
function this._SetUpSoldierTypes(typeName,soldierTypes)
  for a,soldierId in ipairs(soldierTypes)do
    if IsTypeTable(soldierId)then
      this._SetUpSoldierTypes(typeName,soldierId)
    else
      mvars.ene_soldierTypes[soldierId]=EnemyType["TYPE_"..typeName]
    end
  end
end
function this.SetUpSoldierTypes(soldiersTypes)
  for typeName,soldiersTypes in pairs(soldiersTypes)do
    this._SetUpSoldierTypes(typeName,soldiersTypes)
  end
end
function this._SetUpSoldierSubTypes(subType,soldiersTypes)
  for a,soldierName in ipairs(soldiersTypes)do
    if IsTypeTable(soldierName)then
      this._SetUpSoldierSubTypes(subType,soldierName)
    else
...
function this.SetUpSoldierSubTypes(soldierSubTypes)
  for subType,soldierName in pairs(soldierSubTypes)do
    this._SetUpSoldierSubTypes(subType,soldierName)
  end
end
function this.SetUpPowerSettings(powerSettings)
  mvars.ene_missionSoldierPowerSettings=powerSettings
  local requiredPowerSettings={}
  for t,e in pairs(powerSettings)do
    for e,t in pairs(e)do
      local e=e
      if Tpp.IsTypeNumber(e)then
        e=t
      end
      requiredPowerSettings[e]=true
    end
  end
  mvars.ene_missionRequiresPowerSettings=requiredPowerSettings
end
...
--tex DEBUGNOW:
  ...
  --[[DEBUGNOW:if TppMission.IsFOBMission(missionCode)or InfMain.IsMbPlayTime() then--tex
    return"DD_FOB"
  end--]]
  ...
  for equipType,equipmentTable in pairs(weaponIdInfo)do
    for n,equipment in ipairs(equipmentTable)do
      ...
      elseif equipment.isNoKill and not isNoKillMode then
        ...
        --[[DEBUGNOW:if gvars.mbSoldierEquipGrade == InfMain.SETTING_MB_EQUIPGRADE.MAX then--tex
          developRank=1
        end--]]
...
function this.GetBodyId(soldierId,soldierType,soldierSubType,loadout)
  ...
  local _GetBodyId=function(soldierId,loadoutBodyId)
  ...
  if loadout.ARMOR and subTypeBody.ARMOR then
    return _GetBodyId(soldierId,subTypeBody.ARMOR)
  end
  if(mvars.ene_soldierLrrp[soldierId]or loadout.RADIO)and subTypeBody.RADIO then
    return _GetBodyId(soldierId,subTypeBody.RADIO)
  end
  if loadout.MISSILE and subTypeBody.MISSILE then
    return _GetBodyId(soldierId,subTypeBody.MISSILE)
  end
  if loadout.SHIELD and subTypeBody.SHIELD then
    return _GetBodyId(soldierId,subTypeBody.SHIELD)
  end
  if loadout.SNIPER and subTypeBody.SNIPER then
    bodyId=_GetBodyId(soldierId,subTypeBody.SNIPER)
  elseif loadout.SHOTGUN and subTypeBody.SHOTGUN then
    if loadout.OB and subTypeBody.SHOTGUN_OB then
      bodyId=_GetBodyId(soldierId,subTypeBody.SHOTGUN_OB)
    else
      bodyId=_GetBodyId(soldierId,subTypeBody.SHOTGUN)
    end
  elseif loadout.MG and subTypeBody.MG then
    if loadout.OB and subTypeBody.MG_OB then
      bodyId=_GetBodyId(soldierId,subTypeBody.MG_OB)
    else
      bodyId=_GetBodyId(soldierId,subTypeBody.MG)
    end
  elseif subTypeBody.ASSAULT then
    if loadout.OB and subTypeBody.ASSAULT_OB then
      bodyId=_GetBodyId(soldierId,subTypeBody.ASSAULT_OB)
    else
      bodyId=_GetBodyId(soldierId,subTypeBody.ASSAULT)
    end
  end
  return bodyId
end
...
function this.ApplyPowerSetting(soldierId,loadout)
  ...
  local enemySubTypeForSubTypeString={
  ...
  SendCommand(soldierId,{id="SetSoldier2SubType",type=enemySubTypeForSubTypeString[soldierSubType]})
end
...
function this.RegisterSoldier2CommonMotionPackagePath(packageLabel)
  local commonPack=TppDefine.SOLIDER2_COMMON_PACK[packageLabel]
  local commonPackPrerequisites=TppDefine.SOLIDER2_COMMON_PACK_PREREQUISITES[packageLabel]
  if commonPack then
    ...
    commonPack=TppDefine.SOLIDER2_COMMON_PACK.default
    commonPackPrerequisites=TppDefine.SOLIDER2_COMMON_PACK_PREREQUISITES.default
    this.ResetSoldier2CommonBlockPackageLabel()
  end
  TppSoldier2CommonBlockController.SetPackagePathWithPrerequisites{path=commonPack,prerequisites=commonPackPrerequisites}
end
...
function this.DefineSoldiers(soldierDefine)--NMC: from <location>_enemy.lua?, soldierDefine[cpId]={soldierlist}
  ...
  for cpName,soldierList in pairs(soldierDefine)do
    ...
    if cpGameId==NULL_ID then
      ...
      for soldierName,soldierName in pairs(soldierList)do
        if IsTypeString(soldierName)then
          if not this.SOLDIER_DEFINE_RESERVE_TABLE_NAME[soldierName]then
          end
        else
          local soldierGameId=GetGameObjectId(soldierName)
          if soldierGameId==NULL_ID then
          else
            mvars.ene_soldierIDList[cpGameId][soldierGameId]=soldierName
          end
        end
      end
    end
...
function this.FultonRecoverOnMissionGameEnd()
  ...
  for t,gameId in pairs(activeWalkerGears)do
    if IsWithinDistSqr(a,i,gameId)then
      ...
      TppTerminal.OnFulton(gameId,nil,nil,resourceId,true,notHelicopter,PlayerInfo.GetLocalPlayerIndex())
    end
  end
  TppHelicopter.SetNewestPassengerTable()
  ...
  for r,t in pairs(mvars.ene_soldierIDList)do
    for t,r in pairs(t)do
      if IsWithinDistSqr(a,i,t)and(not this.IsQuestNpc(t))then
        this.AutoFultonRecoverNeutralizedTarget(t,notHelicopter)
      end
    end
  end
  local t=this.GetAllHostages()
  for r,gameId in pairs(t)do
    if((not TppHelicopter.IsInHelicopter(gameId))and IsWithinDistSqr(a,i,gameId))and(not this.IsQuestNpc(gameId))then
      local staffId=TppMotherBaseManagement.GetStaffIdFromGameObject{gameObjectId=gameId}
      TppTerminal.OnFulton(gameId,nil,nil,staffId,true,notHelicopter,PlayerInfo.GetLocalPlayerIndex())
    end
  end
  TppHelicopter.ClearPassengerTable()
end
function this.AutoFultonRecoverNeutralizedTarget(gameId,a)
  local n=SendCommand(gameId,{id="GetLifeStatus"})
    ...
    if t.hostageName then
      ...
      if gameId==NULL_ID then
        ...
        if this.CheckQuestDistance(gameId)then
          if TppMission.CheckMissionState(true,false,true,false)then
            local staffId=TppMotherBaseManagement.GetStaffIdFromGameObject{gameObjectId=gameId}
            TppTerminal.OnFulton(gameId,nil,nil,staffId,nil,true)
          end
        end
        if t.bodyId or t.faceId then
        ...
        if this.CheckQuestDistance(gameId)then
          if TppMission.CheckMissionState(true,false,true,false)then
            GameObject.SendCommand(gameId,{id="RequestVanish"})
          end
        end
      end
    end
...
function this.GetDDSuit()
  ...
  --DEBUGNOW:if TppMission.IsFOBMission(vars.missionCode) then--tex isolated whole thing for now RETRY:
...
function this._OnFulton(gameId,a,a,staffId)--RENAME:
  this._OnRecoverNPC(gameId,staffId)
end
