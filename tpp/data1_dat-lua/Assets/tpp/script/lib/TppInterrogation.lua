...
function e.SetQuestHighIntTable(e,a)
  ...
  for a,r in pairs(a)do
    ...
    local e=mvars.interTable[e].index
    local n=svars.InterrogationHigh[e]local a=bit.lshift(1,a-1)svars.InterrogationHigh[e]=bit.bor(n,a)
  end
end
    ...
    for r,n in pairs(mvars.interTable[e].layer.high)do
      if a.name==n.name then
        if a.func==n.func then
          local e=mvars.interTable[e].index
          local n=svars.InterrogationHigh[e]local a=bit.lshift(1,r-1)svars.InterrogationHigh[e]=bit.bor(n,a)
        end
      end
    end
  end
  ...
  for n,a in pairs(a)do
    for r,n in pairs(mvars.interTable[e].layer.high)do
      if a.name==n.name then
        if a.func==n.func then
          local e=mvars.interTable[e].index
          local a=svars.InterrogationHigh[e]local n=bit.lshift(1,r-1)svars.InterrogationHigh[e]=bit.band(a,bit.bnot(n))
        end
      end
    end
  end
end
function e.InitUniqueInterrogation(e)mvars.uniqueInterTable={uniqueChara={},unique={}}
...
function e.AddQuestTable(n)
  if mvars.questTable==nil then
    mvars.questTable={uniqueChara={},unique={}}e.ResetQuestTable()
  end
...
function e.InitInterrogation(e)mvars.interTable={}
  local a=0
...
function e._AssignInterrogation(e,n,a)r(e,{id="AssignInterrogation",messageId=n,index=a})
end
function e._AssignInterrogationCollection(e)r(e,{id="AssignInterrogationCollection"})
end
