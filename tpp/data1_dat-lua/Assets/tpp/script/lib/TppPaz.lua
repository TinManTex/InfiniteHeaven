...
local N="paz_room02"local J="paz_room04"local H={"paz_misete01"}
local x="paz_misete02"local K={"paz_atama01","paz_atama02","paz_atama03","paz_atama04"}
...
local ae="paz_room_book01"local Z="paz_room_book02"local Q="paz_photo_after04"local X={"paz_photo_after01","paz_photo_after02","paz_photo_after03"}
local V={"paz_negoto01","paz_negoto02","paz_negoto03","paz_negoto04"}
local D={"paz_matane01","paz_matane02"}
local p="PazTimerSenderEnableSeeYou"local m="PazTimerSenderEnableShowMe"local v="PazTimerSenderPhotoAfter"local u="PazTimerSenderDown"local h="PazTimerSenderHumming"local f="PazTimerSenderPerceive"local l="PazTimerSenderSleep"local T="PazTimerSenderConfusionOutAngle"local g="PazTimerSenderAimDefault"local z="PazTimerSenderCheckIdleAction"local c="PazTimerSenderEnableIdleAction"local d="PazTimerSenderIdleMonologue"local t="PazTimerSenderEnableIdleMonologue"local M="PazClockSenderEndWait"function this.ActiveMessages()
  return Tpp.StrCode32Table{GameObject={{msg="SpecialActionEnd",sender=o,func=this.OnSpecialActionEnd},{msg="PazShowIcon",sender=o,func=this.OnPazShowIcon},{msg="PazHideIcon",sender=o,func=this.OnPazHideIcon},{msg="PazPerceiveSnake",sender=o,func=this.OnPazPerceiveSnake},{msg="MonologueEnd",sender=o,func=this.OnMonologueEnd},{msg="PazHasAimedDefault",sender=o,func=this.OnPazHasAimedDefault},{msg="PazRelaxEndTiming",sender=o,func=this.OnPazRelaxEndTiming},{msg="PazOutAngle",sender=o,func=this.OnPazOutAngle},{msg="PazInAngle",sender=o,func=this.OnPazInAngle},{msg="PazSnakeIsStopping",sender=o,func=this.OnPazSnakeIsStopping},{msg="PazSnakeIsMoving",sender=o,func=this.OnPazSnakeIsMoving}},Player={{msg="IconOk",func=this.OnIconOk},{msg="IconSwitchShown",func=this.OnIconSwitchShown}},Timer={{msg="Finish",sender=p,func=function()
    this.OnFinishTimer(p)
  end},{msg="Finish",sender=m,func=function()
    this.OnFinishTimer(m)
  end},{msg="Finish",sender=v,func=function()
    this.OnFinishTimer(v)
  end},{msg="Finish",sender=u,func=function()
    this.OnFinishTimer(u)
  end},{msg="Finish",sender=h,func=function()
    this.OnFinishTimer(h)
  end},{msg="Finish",sender=f,func=function()
    this.OnFinishTimer(f)
  end},{msg="Finish",sender=l,func=function()
    this.OnFinishTimer(l)
  end},{msg="Finish",sender=T,func=function()
    this.OnFinishTimer(T)
  end},{msg="Finish",sender=g,func=function()
    this.OnFinishTimer(g)
  end},{msg="Finish",sender=z,func=function()
    this.OnFinishTimer(z)
  end},{msg="Finish",sender=c,func=function()
    this.OnFinishTimer(c)
  end},{msg="Finish",sender=d,func=function()
...
function this.InactiveMessages()
  return Tpp.StrCode32Table{Weather={{msg="Clock",sender=M,func=function()
    this.OnClock(M)
  end}}}
end
...
function this.OnDemoEnter()
  ...
  local n=nil
  local t=nil
  local o=this.GetDemoEndType()
  if o==this.DEMO_END_TYPE_SIT then
    n=a
    t="Book"elseif o==this.DEMO_END_TYPE_DOWN then
    n=A
    t="Blood"end
  if n~=nil then
    this.SendCommandSpecialAction(n,t)
  end
  this.messageExecTable=Tpp.MakeMessageExecTable(this.ActiveMessages())
  this.InitUi()
  this.InitMusic()
...
function this.SendCommandSpecialAction(o,r)
  local t=o[2]
  if not r then
    r=t
  end
  ...
  Y(this.GetPazGameObjectId(),{id="SpecialAction",action=o[1],path=t,state=t,autoFinish=false,enableMessage=true,commandId=n(r),enableGravity=false,enableCollision=false,interpFrame=m})
...
function this.SendCommandCallMonologue(n)
  ...
  Y(this.GetPazGameObjectId(),{id="CallMonologue",label=n,reset=true})
...
function this.PlayPlayerMotion(e)Player.RequestToPlayDirectMotion{"handBookToPaz",{e,false,o,"Move","MTP_GLOBAL_C",false}}
end
function this.PlayCameraAnimation()Player.RequestToPlayCameraNonAnimation{characterId=this.GetPazGameObjectId(),isFollowPos=true,isFollowRot=true,followTime=6,followDelayTime=.5,candidateRots={{-10,110}},skeletonNames={"SKL_002_CHEST"},skeletonCenterOffsets={Vector3(0,0,0)},skeletonBoundings={Vector3(.1,.1,.1)},offsetTarget=Vector3(0,.5,.5),offsetPos=Vector3(0,0,-1),focalLength=21,aperture=4,timeToSleep=6,interpTimeAtStart=1,fitOnCamera=false,timeToStartToFitCamera=1,fitCameraInterpTime=.3,diffFocalLengthToReFitCamera=16,isCollisionCheck=false}Player.RequestToSetCameraRotation{rotX=10,rotY=45,interpTime=1}
end
