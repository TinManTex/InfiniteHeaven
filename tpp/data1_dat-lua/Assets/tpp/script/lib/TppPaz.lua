...
local re="/Assets/tpp/motion/SI_game/fani/bodies/snap/snappaz/snappaz_give_book.gani"local oe="/Assets/tpp/motion/SI_game/fani/bodies/snap/snappaz/snappaz_give_pic.gani"local te={b}
local le={S}
local B={q}
...
e.messageExecTable=Tpp.MakeMessageExecTable(e.ActiveMessages())GkEventTimerManager.Start(p,30)GkEventTimerManager.Start(z,2)GkEventTimerManager.Start(c,10)
...
e.SendCommandCallMonologue(D)GkEventTimerManager.Stop(p)GkEventTimerManager.Start(p,30)GkEventTimerManager.Stop(m)GkEventTimerManager.Start(m,30)
mvars.paz_isSeeYouMonologueEnabled=false
...
Y(e.GetPazGameObjectId(),{id="SpecialAction",action=o[1],path=t,state=t,autoFinish=false,enableMessage=true,commandId=n(r),enableGravity=false,enableCollision=false,interpFrame=m})
...
e.SendCommandCallMonologue(x)GkEventTimerManager.Start(m,30)
mvars.paz_isShowMeMonologueEnabled=false
end
end
else
...
function e.PlayPlayerMotion(e)Player.RequestToPlayDirectMotion{"handBookToPaz",{e,false,o,"Move","MTP_GLOBAL_C",false}}
end
