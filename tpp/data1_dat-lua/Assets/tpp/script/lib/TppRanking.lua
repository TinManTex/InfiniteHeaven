...
local n={[30010]=true,[30020]=true,[30050]=true}e.EXCLUDE_MISSION_LIST={false,n,n,n,n,false,false,false,n,n,false,false,false,false,false,false,false,false,false,false}
...
function e.UpdateScore(t,a,i)
  local n=TppDefine.RANKING_ENUM[t]if not n then
    return
  end
...
function e.SendCurrentRankingScore()
  ...
  for a=1,(TppDefine.RANKING_MAX-1)do
    if svars.rnk_isUpdated[a]then
      local t=TppDefine.RANKING_ENUM[a]local e=e.GetScore(t)table.insert(n,{a,e})
    end
  end
...
function e.CheckExcludeMission(a,n)
  local e=e.EXCLUDE_MISSION_LIST[a]if e then
    return e[n]end
  return false
end
function e.UpdateScoreTime(n)e.UpdateScore(n,svars.scoreTime/1e3)
end
...
function e.Init(n)e.messageExecTable=Tpp.MakeMessageExecTable(e.Messages())
end
function TppResult.OnReload(n)e.Init(n)
end
