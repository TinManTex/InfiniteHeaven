...
sequences.Seq_Demo_ConfirmAutoSave = {
    ...
    --if gvars.startOffline then		--tex seems like theres an engine process that handles popup if in steam offline mode?
    --  TppSequence.SetNextSequence("Seq_Demo_CreateOrLoadSaveData")
    --else
    ...
    --end
		...
		--[[--tex OFF: ORIG:
		if SignIn.PresetUserIdExists and SignIn.PresetUserIdExists() then--tex OFF:
			TppSequence.SetNextSequence("Seq_Demo_StartSignIn")
			return
		end
	...
	OnEnter = function(self)
		...
		local function StateCallback(screenId, state)
			if state == SplashScreen.STATE_DELETE then
				...
				SplashScreen.Show( kjpLogoScreenId, 0, showTime, 0)--tex was 1.0, 4.0, 1.0)
			end
		end
